...
/* 貫通電流が発生するかを検出する */
#define SHORTCURRENT_DETECT(dir_now, dir_last)	((dir_now) != (dir_last) && (dir_now) && (dir_last))
...
{
	u8 port = ch>>2;	// 0:0-3 / 1:4-7 / 2:8-11
	ch &= 0x03; 		// 0-3に制限

	u8 dir_now  = DUTY_TO_DIR(duty);
	u8 dir_last = gMotor_status[port].dir_last[ch];
	...
	{
		if(SHORTCURRENT_DETECT(dir_now, dir_last))
		{
		...
		else
		{
			...
			gMotor_status[port].dir_last[ch] = dir_now;
		}
	}
	else
	{
